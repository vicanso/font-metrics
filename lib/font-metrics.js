class Composing {
  ...
  addChar(ch) {
    const {
      ...
      height,
      getFontSize,
      forbiddenFirstLetters,
    } = this.opts;
    let {
      currentHeight,
      currentWidth,
    } = this;
    const isForbiddenFirstLetters = forbiddenFirstLetters && forbiddenFirstLetters.indexOf(ch) !== -1;
    // if the first char, add the currentHeight
    if (currentWidth === 0) {
      currentHeight += lineHeight;
      // 如果是一行的最开始并且该字符不可显示在第一位
      // 则浏览器会把上一个字符显示到此行，因此宽度需要加上一个字符
      if (isForbiddenFirstLetters) {
        currentWidth += this.prevCharWidth;
      }
    }
    const fontSize = getFontSize(ch);
    this.prevCharWidth = fontSize;
...
class FontMetrics {
  ...
  getComposing() {
    ...
    if (!this.getFontSize) {
      throw new Error('getFontSize function is not defind');
    }
    ...
    const composing = new Composing({
      ...
      forbiddenFirstLetters: opts.forbiddenFirstLetters,
    });
  ...
  toHTML() {
    ...
    const getStyle = (index, total, firstParagraphIndented) => {
      let style = `font-size:${fontSize}px;line-height:${lineHeight}px;margin:0;padding:0;`;
      // 非第一段文章内容 或者每页的第一段，而且是新的段落
